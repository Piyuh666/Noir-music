...
export const HANDLEPLAYERVOLUMEDOWN_CONTRACT = contract(UIAction.PLAYER_VOLUME_DOWN, "button", "volume", true);

export async function handlePlayerVolumeDown(interaction: ButtonInteraction): Promise<void> {
  await executeDedicatedHandler(interaction, HANDLEPLAYERVOLUMEDOWN_CONTRACT, async (_execution) => {
